...
def dashboard_report(request,sap_id):
    if request.method == 'GET':
        sql= "SELECT " \
                ...
                "(SELECT SUM(sis.net_val) net_val, SUM(DISTINCT d.cash_collection) cash_collection, (SUM(sis.net_val) - SUM(DISTINCT d.cash_collection)) due FROM rdl_delivery d INNER JOIN rpl_sales_info_sap sis ON d.billing_doc_no=sis.billing_doc_no WHERE d.billing_date = CURRENT_DATE() AND d.da_code = '%s' AND d.delivery_status = 'Done' AND d.cash_collection_status = 'Done') total_due;"
